    ...
    <div className="max-w-6xl mx-auto px-6 py-8 space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex items-center justify-between border-b border-slate-200 pb-6">
        <div>
          ...
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">Stock Movements</h1>
        </div>

        <button 
          onClick={() => navigate('/reports/stock')}
          className="h-10 px-4 flex items-center gap-2 border border-slate-200 rounded-lg hover:bg-slate-50 transition-all group"
        >
          <History size={14} className="text-slate-400 group-hover:text-indigo-600" />
          <span className="text-[10px] font-black uppercase tracking-widest text-slate-600">Movement Logs</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {actions.map((action) => (
          <button
            ...
            className={cn(
              "group relative bg-white border border-slate-200 rounded-xl p-6 text-left transition-all overflow-hidden",
              "hover:shadow-md active:scale-[0.99]",
              action.border
            )}
          >
            {/* Background Identifier Decor */}
            ...
            <div className="space-y-1 mb-4">
              <h2 className="text-xl font-black text-slate-900 tracking-tight uppercase italic">{action.title}</h2>
      ...
      <div className="border border-slate-200 rounded-xl bg-slate-50/50 p-2 flex flex-col md:flex-row gap-2">
...
function MonitorCard({ label, value, isWarning, isStatus }: { label: string, value: string, isWarning?: boolean, isStatus?: boolean }) {
  return (
    <div className="flex-1 bg-white border border-slate-100 rounded-lg p-3 flex items-center justify-between px-4">
