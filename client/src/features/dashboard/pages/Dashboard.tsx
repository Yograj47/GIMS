...
export default function Dashboard() {
    ...
    const { activeAlerts, fetchActiveAlerts, markAsResolved } = useAlerts();
    ...
    return (
        /* Changed to h-full and used space-y-8 for consistent layout spacing */
        <div className="h-full space-y-8 animate-in fade-in duration-700">
            
            {/* 1. Page Header - Aligned with ManageProduct style */}
            <div className="flex items-center justify-between border-b border-slate-200 pb-8">
                ...
                {/* Optional: Add a "Last Updated" timestamp here for that professional feel */}
                ...
                <div className="lg:col-span-3">
                    <div className="sticky top-8 space-y-6">
                        <div className="border-l-4 border-blue-600 pl-4 py-1">
                            <h3 className="text-[11px] font-black text-slate-900 uppercase tracking-[0.2em]">Quick Actions</h3>
                            <p className="text-[9px] font-bold text-slate-400 uppercase">Command Center</p>
                        </div>
                        
                        <div className="space-y-3">
                            <ActionTile icon={<Plus size={18} />} label="New Product" color="bg-blue-600" onClick={() => navigate('/products/new')} />
                            <ActionTile icon={<ArrowUpRight size={18} />} label="Record Sale" color="bg-emerald-500" onClick={() => navigate('/sales/new')} />
                            <ActionTile icon={<Truck size={18} />} label="Suppliers" color="bg-slate-900" onClick={() => navigate('/suppliers')} />
                        </div>

                        {/* Dashboard Tip/Info Card - Flattened slightly for precision look */}
                        <div className="p-6 bg-slate-900 rounded-sm text-white shadow-xl shadow-slate-200 mt-10 relative overflow-hidden">
                            <h4 className="font-black text-[10px] uppercase tracking-[0.2em] mb-3 text-blue-400">Pro Analysis</h4>
                            ...
                            <div className="absolute -right-4 -top-4 h-16 w-16 bg-blue-600/10 rounded-full" />
                        </div>
                    </div>
                </div>
            </div>
